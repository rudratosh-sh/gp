@extends('doctor.layouts.doctor-layout', ['active' => 'history','page_title'=>'Create A Other Info'])
@section('content')
    <style>
        .right-hr {
    ...
    <div class="content">
        <section class="bg-grey1 patient-detail-wrapper flex">
            ...
            <div class="right_section">
                <form method="POST" action="{{ route('doctor.create.other-info.post') }}" enctype="multipart/form-data">
                    @csrf
                    <div class="flex items-center justify-between">
                        <div class="flex items-center">
                            <img width="24px" height="24px" src="/assets/images/create-letter.svg" alt="" />
                            <p class="text-grey2 text-20 font-bold ml-10">Create Other</p>
                        </div>
                        <div class="flex items-center">
                    ...
                    <div class="form-second-sec bg-white flex flex-col justify-between">
                        <div class="flex flex-col">
                            ...
                            <div class="mt-36">
                                ...
                                <textarea class="input text-grey2 text-xs font-normal" placeholder="Type Here" name="presenting_complaints">{{ optional($appointment->otherInfo)->presenting_complaints }}                                </textarea>
                            </div>
                            <div class="mt-36">
                                <p class="text-grey2 text-xs font-normal">Relevant History</p>
                                <textarea class="input text-grey2 text-xs font-normal" placeholder="Type Here" name="relevant_history">{{ optional($appointment->otherInfo)->relevant_history }} </textarea>
                            </div>

                            <div class="mt-36">
                                <p class="text-grey2 text-xs font-normal">Examination</p>
                                <textarea class="input text-grey2 text-xs font-normal" placeholder="Type Here" name="examination">{{ optional($appointment->otherInfo)->examination }} </textarea>
                            </div>

                            <div class="mt-36">
                                <p class="text-grey2 text-xs font-normal">Recommendation</p>
                                <textarea class="input text-grey2 text-xs font-normal" placeholder="Type Here" name="recommendation">{{ optional($appointment->otherInfo)->recommendation }} </textarea>
                            </div>

                            <div class="mt-36">
                                <p class="text-grey2 text-xs font-normal">Followup</p>
                                <textarea class="input text-grey2 text-xs font-normal" placeholder="Type Here" name="followup">{{ optional($appointment->otherInfo)->followup }} </textarea>
                            </div>
                            ...
                            <div class="mt-36">
                                ...
                                <textarea class="input text-grey2 text-xs font-normal" placeholder="Type Here" name="personalization_framework">{{ optional($appointment->otherInfo)->personalization_framework }} </textarea>
                            </div>
                            <input type="hidden" value="{{ encrypt($appointment->user->id) }}" name="other_user_id" />
                            <input type="hidden" value="{{ encrypt($appointment->clinic_id) }}"
                                name="other_clinic_id" />
                            <input type="hidden" value="{{ encrypt($appointment->doctor_id) }}"
                                name="other_doctor_id" />
